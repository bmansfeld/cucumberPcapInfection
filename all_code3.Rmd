...
```{r moduleskME}
geneMod4 <-
  tibble(gene = colnames(infection16dpp)[moduleLabels16 == 4]) %>%
  mutate(
    modMembership = abs(geneModuleMembership16[gene, "ME4"]),
    modMemAdjPval = p.adjust(p = MMPvalue16$ME5, method = "BH")[moduleLabels16 == 4]
  ) %>%
  left_join(resultsEarly)

geneMod7 <-
  tibble(gene = colnames(infection16dpp)[moduleLabels16 == 7]) %>%
  mutate(
    modMembership = abs(geneModuleMembership16[gene, "ME7"]),
    modMemAdjPval = p.adjust(p = MMPvalue16$ME9, method = "BH")[moduleLabels16 == 7]
  ) %>%
  left_join(resultsEarly)
```
...
mod5and9GOIs <- bind_rows("Mod5" = geneMod4_filt, "Mod9" = geneMod7_filt, .id = "Module")

kableExtra::kable(mod5and9GOIs %>% 
