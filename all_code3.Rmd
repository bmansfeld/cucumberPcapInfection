...
cowplot::plot_grid(T4venn, T24venn, T48venn, 
                   labels = "AUTO", align = "h", ncol = 1)
```
...
Let's examine the module preservation
```{r modPres, message=FALSE, warning=FALSE}
...
Export GO term enrichment results for Resistant network modules

```{r exportGO, message=FALSE, warning=FALSE}
GOres <- list()
k <- 1
...
write_xlsx(GOres, "Resistant_Modules_GO_Enrichment.xlsx", col_names = T)
...
# fills <- as_tibble(rev(names(sort(table(moduleColors16.custom), decreasing = T))[anovaModExp$moduleLabel])) %>% mutate(moduleLabel = as.factor(row_number()))
...
moduleCount %>%
  ...
  stat_summary(fun.data = "mean_se", geom = "errorbar",
    ...
    color = rep(rep(c("#00BFC4", "#00BFC4", "#F8766D", "#F8766D"), each = 7), 3)
  ) +
  scale_x_continuous(breaks = c(0, 2, 4, 8, 12, 18, 24)) +
  labs(x = "Hours post inoculation", y = "Normalized read count") +
  theme_gray() +
