...
```{r prepnetworks}
# row variance for all inoculated samples
# rv16 <- rowVars(assay(vst)[, vst$treatment == "Inoc" & vst$age == "16dpp"])
# rv8 <- rowVars(assay(vst)[, vst$treatment == "Inoc" & vst$age == "8dpp"])
# 
# 
# # select the 5000 genes by variance
# top <- 5000
# 
# select16 <- order(rv16, decreasing=TRUE)[seq_len(min(top, length(rv16)))]
# select8 <- order(rv8, decreasing=TRUE)[seq_len(min(top, length(rv8)))]
# 
# select_union <- union(select16, select8)
